...
<span class="token function">mount</span> /dev/mapper/my_encrypted_disk /mnt</code>`),a(x);var S=s(x,12),ts=n(S);p(ts,()=>`<code class="language-shell"><span class="token function">apt</span> <span class="token function">install</span> ufw
ufw allow <span class="token function">ssh</span> <span class="token comment"># If you don't want to secure SSH with VPN</span>
ufw default deny incoming
ufw default allow outgoing</code>`),a(S);var I=s(S,6),z=s(n(I));r(z,{href:"https://github.com/angristan/wireguard-install>wireguard-install",children:(t,i)=>{e();var o=l("wireguard-install");c(t,o)}});var os=s(z,2);r(os,{href:"https://github.com/donaldzou/WGDashboard",children:(t,i)=>{e();var o=l("WGDashboard");c(t,o)}}),e(),a(I);var P=s(I,4),ps=n(P);p(ps,()=>`<code class="language-shell"><span class="token function">apt</span> <span class="token function">install</span> <span class="token function">curl</span>
...
./wireguard-install.sh</code>`),a(P);var A=s(P,10),cs=n(A);p(cs,()=>`<code class="language-shell">ufw allow <span class="token operator">&lt;</span>WIREGUARD_PORT<span class="token operator">></span>/udp
...
ufw <span class="token builtin class-name">enable</span></code>`),a(A);var T=s(A,8),ls=s(n(T));r(ls,{href:"https://letsencrypt.org/",children:(t,i)=>{e();var o=l("Let’s Encrypt");c(t,o)}}),e(),a(T);var C=s(T,4),rs=n(C);p(rs,()=>'<code class="language-shell"><span class="token function">apt</span> <span class="token function">install</span> certbot</code>'),a(C);var N=s(C,4),is=n(N);p(is,()=>'<code class="language-shell">certbot certonly <span class="token parameter variable">--standalone</span> --config-dir /mnt/nginx/ssl/config --work-dir /mnt/nginx/ssl --logs-dir /mnt/nginx/logs <span class="token parameter variable">-d</span> yourdomain.com </code>'),a(N);var H=s(N,4),us=s(n(H));r(us,{href:"https://docs.portainer.io/",children:(t,i)=>{e();var o=l("Portainer CE");c(t,o)}}),e(),a(H);var E=s(H,4),ks=n(E);p(ks,()=>`<code class="language-shell"><span class="token function">apt</span> <span class="token function">install</span> docker.io <span class="token function">docker-compose</span>
systemctl <span class="token builtin class-name">enable</span> <span class="token parameter variable">--now</span> <span class="token function">docker</span></code>`),a(E);var D=s(E,4),ds=n(D);p(ds,()=>`<code class="language-yaml"><span class="token key atrule">services</span><span class="token punctuation">:</span>
...
<span class="token key atrule">volumes</span><span class="token punctuation">:</span> <span class="token comment"># If you want to store the data on a different drive, see https://github.com/nextcloud/all-in-one#how-to-store-the-filesinstallation-on-a-separate-drive</span>
  <span class="token key atrule">nextcloud_aio_mastercontainer</span><span class="token punctuation">:</span>
    ...
    <span class="token key atrule">driver_opts</span><span class="token punctuation">:</span>
      ...
      <span class="token key atrule">o</span><span class="token punctuation">:</span> bind</code>`),a(D);var $=s(D,4),ms=n($);p(ms,()=>`<code class="language-nginx"><span class="token directive"><span class="token keyword">user</span>  nginx</span><span class="token punctuation">;</span>
...
<span class="token punctuation">&#125;</span></code>`),a($);var R=s($,4),ys=n(R);p(ys,()=>`<code class="language-shell"><span class="token function">mkdir</span> <span class="token parameter variable">-p</span> /mnt/nginx/ssl /mnt/nginx/ssl/config /mnt/nginx/logs /mnt/nginx/conf.d
<span class="token function">curl</span> <span class="token parameter variable">-L</span> https://ssl-config.mozilla.org/ffdhe2048.txt <span class="token parameter variable">-o</span> /mnt/nginx/ssl/dhparam</code>`),a(R);var G=s(R,6),hs=n(G);p(hs,()=>`<code class="language-nginx"><span class="token directive"><span class="token keyword">map</span> <span class="token variable">$http_upgrade</span> <span class="token variable">$connection_upgrade</span></span> <span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`),a(G);var V=s(G,10),fs=n(V);p(fs,()=>`<code class="language-nginx"><span class="token directive"><span class="token keyword">map</span> <span class="token variable">$http_upgrade</span> <span class="token variable">$connection_upgrade</span></span> <span class="token punctuation">&#123;</span>
    <span class="token directive"><span class="token keyword">default</span> upgrade</span><span class="token punctuation">;</span>
    '' <span class="token directive"><span class="token keyword">close</span></span><span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span>
...
<span class="token directive"><span class="token keyword">server</span></span> <span class="token punctuation">&#123;</span>
    ...
    <span class="token directive"><span class="token keyword">ssl_conf_command</span> Options PrioritizeChaCha</span><span class="token punctuation">;</span>
    <span class="token directive"><span class="token keyword">ssl_ciphers</span> TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256:ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-RSA-CHACHA20-POLY1305:ECDHE-RSA-AES128-GCM-SHA256</span><span class="token punctuation">;</span>
<span class="token punctuation">&#125;</span></code>`),a(V);var O=s(V,6),gs=n(O);p(gs,()=>`<code class="language-shell">ufw allow from any to <span class="token operator">&lt;</span>WIREGUARD_IP<span class="token operator">></span> port <span class="token number">13000</span>  <span class="token comment"># Allow vaultwarden on VPN</span>
...
<span class="token function">docker-compose</span> up <span class="token parameter variable">-d</span></code>`),a(O);var U=s(O,22),vs=n(U);p(vs,()=>'<code class="language-shell"><span class="token function">apt</span> <span class="token function">install</span> unbound</code>'),a(U);var L=s(U,4),bs=n(L);p(bs,()=>`<code class="language-yaml"><span class="token key atrule">include-toplevel</span><span class="token punctuation">:</span> <span class="token string">"/etc/unbound/unbound.conf.d/*.conf"</span>
<span class="token key atrule">server</span><span class="token punctuation">:</span>
    ...
    <span class="token key atrule">msg-cache-slabs</span><span class="token punctuation">:</span> <span class="token number">4</span></code>`),a(L);var j=s(L,4),_s=n(j);p(_s,()=>`<code class="language-shell"><span class="token function">curl</span> <span class="token parameter variable">-s</span> <span class="token parameter variable">-S</span> <span class="token parameter variable">-L</span> https://raw.githubusercontent.com/AdguardTeam/AdGuardHome/master/scripts/install.sh <span class="token operator">|</span> <span class="token function">sh</span> <span class="token parameter variable">-s</span> -- <span class="token parameter variable">-v</span>
/opt/AdGuardHome/AdGuardHome <span class="token parameter variable">-s</span> <span class="token function">install</span>
/opt/AdGuardHome/AdGuardHome <span class="token parameter variable">-s</span> start</code>`),a(j),e(14),c(m,q)},$$slots:{default:!0}}))}const Xs=Object.freeze(Object.defineProperty({__proto__:null,default:Hs,metadata:W},Symbol.toStringTag,{value:"Module"}));export{Xs as _,Hs as a};
